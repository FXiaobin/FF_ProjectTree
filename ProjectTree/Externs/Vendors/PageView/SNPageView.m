...
- (instancetype)initWithFrame:(CGRect)frame p_style:(PageViewStyle)p_style {
    ...
    if (self) {
        ...
        _topTitleViewColor = [UIColor whiteColor];
        _titleNormalColor = RGBColor(225, 64, 65);
        _titleSelectedColor = [UIColor lightGrayColor];
        _sliderColor = [UIColor whiteColor];
        _titleNormalFont = [UIFont systemFontOfSize:14];
        _titleSelectedFont = [UIFont systemFontOfSize:18];
    }
    return self;
}
...
- (void)startAddSubView {
    ...
    for (NSInteger i = 0; i < count; i++) {
        ...
        btn.frame = CGRectMake(_titleWidth * i, 0, _titleWidth, _tilteHeght);
        ...
        if ([obj isKindOfClass:[UIViewController class]]) {
            ...
            subView = [[obj alloc] init];
        }
        
        subView.frame = CGRectMake(_pageFrame.size.width * i, 0, _pageFrame.size.width, _pageFrame.size.height - _tilteHeght);
...
- (void)titleButtonClick:(UIButton *)btn {
    ...
    [_containerScrollView setContentOffset:CGPointMake(_pageFrame.size.width *(btn.tag - kBtnTag),0) animated:YES];
    if (self.pageViewActionBlock) {
        self.pageViewActionBlock(_selectedButton,_selectedButton.tag - kBtnTag);
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if (scrollView == _containerScrollView) {
        ...
        UIButton * btn = [_topScrollView viewWithTag:offSetCount + kBtnTag];
